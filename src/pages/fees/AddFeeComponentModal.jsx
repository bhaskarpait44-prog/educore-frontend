// src/pages/fees/AddFeeComponentModal.jsx
...
const schema = z.object({
  ...
  frequency : z.enum(['monthly','quarterly','annual','one_time'], { required_error: 'Frequency is required' }),
  due_day   : z.string().refine(v => {
    const n = parseInt(v)
    return n >= 1 && n <= 28
...
const FREQUENCY_OPTIONS = [
  ...
  { value: 'one_time',  label: 'One Time — admission / exam fee' },
]
...
const AddFeeComponentModal = ({ open, onClose, sessionId, classId: preSelectedClass }) => {
  ...
  useEffect(() => {
    if (open) reset({ name: '', amount: '', frequency: 'monthly', due_day: '10', class_id: preSelectedClass || '' })
  }, [open, preSelectedClass])
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={handleSubmit(onSubmit)} loading={isSaving}>Save Component</Button>
        </>
      }
    >
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Input
          ...
          hint="Day of month payment is due (1–28). Capped at 28."
          {...register('due_day')}
        />
        ...
        >
          💡 After adding components, click <strong>Generate Invoices</strong> to create invoice records for all enrolled students.
        </div>
      </form>
    </Modal>
  )
